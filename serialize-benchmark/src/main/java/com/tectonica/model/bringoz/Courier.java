...
import com.tectonica.model.bringoz.CourierProfile.VehicleType;
import com.tectonica.serialize.EXTERN;

@Buzzable
public class Courier implements Externalizable
{
	...
	{
		EXTERN.writeString(out, cid);
		EXTERN.writeExternalizable(out, profile);
		EXTERN.writeExternalizable(out, location);
		EXTERN.writeEnum(out, status);
		EXTERN.writeString(out, assignedDlvid);
		EXTERN.writeDate(out, lastLocationTime);
		EXTERN.writeDate(out, lastStatusTime);
		EXTERN.writeDate(out, createdTime);
	}

	@Override
	public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException
	{
		cid = EXTERN.readString(in);
		profile = EXTERN.readExternalizable(in, CourierProfile.class);
		location = EXTERN.readExternalizable(in, LatLng.class);
		status = EXTERN.readEnum(in, CourierStatus.values());
		assignedDlvid = EXTERN.readString(in);
		lastLocationTime = EXTERN.readDate(in);
		lastStatusTime = EXTERN.readDate(in);
		createdTime = EXTERN.readDate(in);
	}
}
