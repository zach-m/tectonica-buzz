 ...
 * <p>
 * The byte[] buffer may be modified and then returned to its original state during some read operations, so the same byte[] should not be
 * used concurrently in separate threads.
 * 
 * @author Nathan Sweet <misc@n4te.com>
 */
public class BuzzInputStream extends InputStream
{
	 ...
	 */
	public BuzzInputStream(int bufferSize)
	{
	 ...
	 */
	public BuzzInputStream(byte[] buffer)
	{
	 ...
	 *            An exception is thrown if more bytes than this are read.
	 */
	public BuzzInputStream(byte[] buffer, int offset, int count)
	{
		setBuffer(buffer, offset, count);
	}

	/** Creates a new Input for reading from an InputStream with a buffer size of 4096. */
	public BuzzInputStream(InputStream inputStream)
	{
		this(4096);
		if (inputStream == null)
	...
	public BuzzInputStream(InputStream inputStream, int bufferSize)
	{
		this(bufferSize);
		if (inputStream == null)
			throw new IllegalArgumentException("inputStream cannot be null.");
	...
	public void skip(int count) throws BuzzException
	{
	 ...
	 */
	protected int fill(byte[] buffer, int offset, int count) throws BuzzException
	{
		if (inputStream == null)
			return -1;
		try
	...
	/**
	 * @param required
	 *            Must be > 0. The buffer is filled until it has at least this many bytes.
	 * @return the number of bytes remaining.
	 * @throws BuzzException
	 *             if EOS is reached before required bytes are read (buffer underflow).
	 */
	protected int require(int required) throws BuzzException
	{
		int remaining = limit - position;
		if (remaining >= required)
			return remaining;
		if (required > capacity)
			throw new BuzzException("Buffer too small: capacity: " + capacity + ", required: " + required);
		...
		if (remaining > 0)
		{
			...
			if (count == -1)
				throw new BuzzException("Buffer underflow.");
			remaining += count;
			if (remaining >= required)
			{
				limit += count;
	 ...
	 */
	private int optional(int optional) throws BuzzException
	{
		int remaining = limit - position;
	...
	@Override
	public int read() throws BuzzException
	{
		if (optional(1) <= 0)
			return -1;
	 ...
	 */
	@Override
	public int read(byte[] bytes) throws BuzzException
	{
	 ...
	 */
	@Override
	public int read(byte[] bytes, int offset, int count) throws BuzzException
	{
		if (bytes == null)
			throw new IllegalArgumentException("bytes cannot be null.");
	...
	/** Discards the specified number of bytes. */
	@Override
	public long skip(long count) throws BuzzException
	{
	...
	/** Reads a single byte. */
	public byte readByte() throws BuzzException
	{
		require(1);
	...
	public int readByteUnsigned() throws BuzzException
	{
		require(1);
		return buffer[position++] & 0xFF;
	}
	...
	public byte[] readBytes(int length) throws BuzzException
	{
	...
	public void readBytes(byte[] bytes) throws BuzzException
	{
		readBytes(bytes, 0, bytes.length);
	}

	/** Reads count bytes and writes them to the specified byte[], starting at offset. */
	public void readBytes(byte[] bytes, int offset, int count) throws BuzzException
	{
		if (bytes == null)
			throw new IllegalArgumentException("bytes cannot be null.");
		int copyCount = Math.min(limit - position, count);
	...
	/** Reads a 4 byte int. */
	public int readInt() throws BuzzException
	{
		require(4);
	...
	public int readVarInt(boolean optimizePositive) throws BuzzException
	{
		if (require(1) < 5)
	...
	public boolean canReadInt() throws BuzzException
	{
		if (limit - position >= 5)
			return true;
		if (optional(5) <= 0)
	...
	public boolean canReadLong() throws BuzzException
	{
		if (limit - position >= 9)
			return true;
		if (optional(5) <= 0)
	 ...
	 * {@link BuzzOutputStream#writeString(String)} , {@link BuzzOutputStream#writeString(CharSequence)}, and
	 * {@link BuzzOutputStream#writeAscii(String)}.
	 * 
	 * @return May be null.
	 */
	...
	public float readFloat() throws BuzzException
	{
		return Float.intBitsToFloat(readInt());
	}

	/** Reads a 1-5 byte float with reduced precision. */
	public float readFloat(float precision, boolean optimizePositive) throws BuzzException
	{
	...
	public short readShort() throws BuzzException
	{
		require(2);
	...
	public int readShortUnsigned() throws BuzzException
	{
		require(2);
	...
	public long readLong() throws BuzzException
	{
		require(8);
		byte[] buffer = this.buffer;
	...
	public long readVarLong(boolean optimizePositive) throws BuzzException
	{
		if (require(1) < 9)
	...
	public boolean readBoolean() throws BuzzException
	{
		require(1);
	...
	/** Reads a 2 byte char. */
	public char readChar() throws BuzzException
	{
		require(2);
	...
	public double readDouble() throws BuzzException
	{
		return Double.longBitsToDouble(readLong());
	}

	/** Reads a 1-9 byte double with reduced precision. */
	public double readDouble(double precision, boolean optimizePositive) throws BuzzException
	{
	...
	public int[] readInts(int length, boolean optimizePositive) throws BuzzException
	{
	...
	public long[] readLongs(int length, boolean optimizePositive) throws BuzzException
	{
	...
	/** Bulk input of an int array. */
	public int[] readInts(int length) throws BuzzException
	{
		int[] array = new int[length];
		for (int i = 0; i < length; i++)
			array[i] = readInt();
	...
	/** Bulk input of a long array. */
	public long[] readLongs(int length) throws BuzzException
	{
		long[] array = new long[length];
		for (int i = 0; i < length; i++)
			array[i] = readLong();
	...
	public float[] readFloats(int length) throws BuzzException
	{
	...
	public short[] readShorts(int length) throws BuzzException
	{
	...
	public char[] readChars(int length) throws BuzzException
	{
	...
	public double[] readDoubles(int length) throws BuzzException
	{
